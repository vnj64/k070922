...
func GenerateAuthToken(userUUID string, role permissions.Role, cfg infra.Config) (string, error) {
	expHour, err := strconv.Atoi(cfg.JwtAccessExpireMinutes())
	if err != nil {
		log.Printf("wrong expire time: %v", cfg.JwtAccessExpireMinutes())
	...
	privateKey, err := ParsePrivate(cfg.JwtPrivatePemPath())
	if err != nil {
